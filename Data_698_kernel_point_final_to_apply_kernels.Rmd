...
transfer_set<-read.csv('C:/Users/dawig/Desktop/Data698/Maryland/python_files/r_files/kernel_point_set.csv')

ggplot(data=edges_graph_path) +
  theme(text = element_text(family="corben",color='#249382',size=22),panel.background = element_rect(fill = '#f4f4ef'),panel.grid.major = element_blank(), panel.grid.minor = element_blank())+ geom_circle(aes(x0=-77.21, y0=39.15, r=.32))+geom_point(data=transfer_set,aes(x=longitude,y=latitude),alpha=.3,size=2,color='#3771ce')+ geom_circle(aes(x0=-77.2, y0=39.1, r=.01983))+ geom_circle(aes(x0=-77.2, y0=39.1397, r=.01983))+ geom_circle(aes(x0=-77.2, y0=39.1793, r=.01983))
...
max(transfer_set['longitude'])
min(transfer_set['longitude'])
max(transfer_set['latitude'])
min(transfer_set['latitude'])


acc_kernel_set<-read.csv('C:/Users/dawig/Desktop/Data698/Maryland/notes/accident_more_info/divided_kernel.csv')
transfer_set<-read.csv('C:/Users/dawig/Desktop/Data698/Maryland/kernels/stopping_violations_kernel.csv')
#transfer_set<-transfer_set[8500:12500,]
#transfer_set$kernel_raw<-acc_kernel_set$x

ggplot(data=edges_graph_path) +
  theme(text = element_text(family="corben",color='#249382',size=22),panel.background = element_rect(fill = '#f4f4ef'),panel.grid.major = element_blank(), panel.grid.minor = element_blank(),legend.position="none")+ geom_circle(aes(x0=-77.21, y0=39.15, r=.32))+geom_point(data=transfer_set,aes(x=longitude,y=latitude,color=kernel_raw^.2),alpha=.3,size=2)
...
min(transfer_set['longitude'])
max(transfer_set['longitude'])
print(head(transfer_set['kernel_raw']))

```  
  
  
```{r data-merge,fig.showtext = TRUE,eval=FALSE,fig.height=8,fig.width=8,warning=FALSE,eval=TRUE}
...
```{r create_kernel_blanks,fig.showtext = TRUE,eval=FALSE,fig.height=8,fig.width=8,warning=FALSE}
kernel_point_set<-read.csv('C:/Users/dawig/Desktop/Data698/Maryland/python_files/r_files/kernel_point_set.csv')
head(kernel_point_set)
kernel_point_set[,1:2] %>% 
  mutate(kernel_raw = -10) %>% 
  mutate(kernel_density = -10)->kernel_point_set
head(kernel_point_set)
#violations sets and accident set to make kernel for:
data_input_set<-c('registration', 'speeding', 'equipment', 'traffic_sign', 'stopping_violations', 'alcohol', 'cellphone_and_distracted_driving', 'seatbelt', 'lane_use', 'turning', 'aggressive_driving', 'scene_of_accident', 'right_side_violations', 'emergency_vehicle_violations', 'violations_by_pedestrians', 'parking', 'following_closely', 'driver_care_for_pedestrian', 'right-of-way', 'wrong_way', 'slow_driving', 'school_bus','miscellaneous','accidents')
#write_file<-paste0('C:/Users/dawig/Desktop/Data698/Maryland/kernels/',data_input,'blank.csv')
#write.csv(transfer_set[,-1],write_file,row.names=FALSE)
length(data_input_set)
for (i in 1:24){
  data_input<-data_input_set[i]
  write_file<-paste0('C:/Users/dawig/Desktop/Data698/Maryland/kernels/',data_input,'_blank.csv')
  #write.csv(kernel_point_set,write_file,row.names=FALSE)
}
```
